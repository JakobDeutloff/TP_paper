...
# impact in GtC
K_PFTP = np.array([(K_max['PFAT'] + K_min['PFAT'])/2])
K_AMAZ = np.array([0])
K_PFAT = np.array([0])
...
par_mean = minimize(calibrate, x0=np.array([8.5 / Ts_mean['PFTP']]), method='Nelder-Mead',
                    bounds=[[0.01, 0.5]],
                    tol=1e-6, args=[Ts_mean['PFTP']])

par_min = minimize(calibrate, x0=np.array([8.5 / Ts_min['PFTP']]), method='Nelder-Mead',
                   bounds=[[0.01, 1]],
                   tol=1e-6, args=[Ts_min['PFTP']])

par_max = minimize(calibrate, x0=np.array([8.5 / Ts_max['PFTP']]), method='Nelder-Mead',
                   bounds=[[0.01, 0.5]],
                   tol=1e-6, args=[Ts_max['PFTP']])
...
axes[0].plot(result_min['tipping_elements'].xs(('PFTP', 'C_stock'), level=(1, 2), axis=1))
time = find_threshold(result_min['tipping_elements'].xs(('PFTP', 'C_stock'), level=(1, 2), axis=1), 0.995) - \
       find_trg_year(result_min['tipping_elements'].xs(('PFTP', 'bool_tip'), level=(1, 2), axis=1))
axes[0].set_title('Timescale = ' + str(time) + ' Years')

axes[1].plot(result_mean['tipping_elements'].xs(('PFTP', 'C_stock'), level=(1, 2), axis=1))
time = find_threshold(result_mean['tipping_elements'].xs(('PFTP', 'C_stock'), level=(1, 2), axis=1), 0.995) - \
       find_trg_year(result_mean['tipping_elements'].xs(('PFTP', 'bool_tip'), level=(1, 2), axis=1))
axes[1].set_title('Timescale = ' + str(time) + ' Years')

axes[2].plot(result_max['tipping_elements'].xs(('PFTP', 'C_stock'), level=(1, 2), axis=1))
time = find_threshold(result_max['tipping_elements'].xs(('PFTP', 'C_stock'), level=(1, 2), axis=1), 0.995) - \
       find_trg_year(result_max['tipping_elements'].xs(('PFTP', 'bool_tip'), level=(1, 2), axis=1))
...
ts_coarse = np.array([Ts_min['PFTP'], Ts_mean['PFTP'], Ts_max['PFTP']])
rate_coarse = np.array([par_min.x[0], par_mean.x[0], par_max.x[0]])

ts_fine = np.arange(Ts_min['PFTP'], Ts_max['PFTP'], 3)
ts_fine = np.append(ts_fine, [300])
...
for i in range(len(ts_fine)):
    if error[i] > 0:
        par = minimize(calibrate, x0=np.array([0.01]), method='Nelder-Mead',
                       bounds=[[rate_coarse[2], rate_coarse[0]]],
                       tol=1e-6, args=[ts_fine[i]])
...
rate_ts_PFTP = pd.DataFrame(data=np.array([ts_fine, rate_fine, error]).T, columns=['timescale', 'rate', 'error'])
rate_ts_PFTP.to_csv('Data/Calibration/R_Ts_E_PFTP.csv')
