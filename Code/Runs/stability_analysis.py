"""
Script to generate 20 coupled ensembles with 5000 members each for the stability analysis of our results.
"""
import glob
...
def load_fair_params_unconstrained():
    ...
    return FULL_ensemble_selection, ALL_mems, param_categories


# We split this up into two functions here to save some computing time - only this function needs to be called for every
# ensemble member
def load_fair_params_stab_an(FULL_ensemble_selection, ALL_mems, param_categories, N_sets, ens_name, ens_mem):
    ...
    pickle.dump(CONSTRAINED_thermal_set, open('Data/Params/' + ens_name + '/thermal_set' + str(ens_mem) + '.pkl', 'wb'))
    pickle.dump(CONSTRAINED_gas_set, open('Data/Params/' + ens_name + '/gas_set' + str(ens_mem) + '.pkl', 'wb'))
    pickle.dump(CONSTRAINED_extforc_sfs, open('Data/Params/' + ens_name + '/extforc' + str(ens_mem) + '.pkl', 'wb'))

    return CONSTRAINED_thermal_set, CONSTRAINED_gas_set, CONSTRAINED_extforc_sfs, param_categories


def load_TE_params_stab_an(elements, N_sets, ens_name, ens_mem):
    ...
    pickle.dump(TE_params, open('Data/Params/' + ens_name + '/TE_params' + str(ens_mem) + '.pkl', 'wb'))
    pickle.dump(P_sample, open('Data/Params/' + ens_name + '/P_sample' + str(ens_mem) + '.pkl', 'wb'))

    return TE_params, P_sample


def run_ssp_stab_an(ssp, ens_mem, use_TE_model, thermal_set, gas_set, extforc_sfs, TE_params, P_sample, N_sets,
                    param_categories,
    ...
    # Load total erf timeseries - not the RMIP dataset use in the default FAiR setting and the calibration of the
    # permafrost module but nearly no difference.
    ssp_erf = pd.read_csv(
    ...
    members = result['T'].columns.values
    for element in elements:
        for i in range(len(members)):
            try:
                tip_year = result['T'].index[np.where(result['T'][members[i]] > P_sample[element][i])[0][0]]
                tip_prob_total.loc[tip_year:, (str(ens_mem), element)] += 1
            except:
                pass

    return T, rf, C, alpha_C, alpha_m, Emissions, result['tipping_elements'], TE_output, tip_prob_total


def run_fair_stab_an(ssp, elements, N_sets, N_ens, ens_name, use_TE_model):
    #  Initialize output arrays SSP ensemble
    ens = [str(x) for x in range(N_ens)]
    quant_T = []
    quant_RF = []
    quant_C = []
    quant_alpha_C = []
    ...
    quant_TE = []
    total_TE = []
    multicol = pd.MultiIndex.from_product([ens, elements])
    tip_prob_total = pd.DataFrame(data=np.zeros((SSP_emms.index.__len__(), ens.__len__() * elements.__len__())),
                                  index=SSP_emms.index, columns=multicol)

    # get full parameters of fair
    FULL_ensemble_selection, ALL_mems, param_categories = load_fair_params_unconstrained()

    for i in range(N_ens):
        # select constrained fair parameters
        thermal_set, gas_set, extforc_sfs, param_categories = \
            load_fair_params_stab_an(FULL_ensemble_selection, ALL_mems, param_categories, N_sets, ens_name, i)
        # Get TE model params
        TE_params, P_sample = load_TE_params_stab_an(elements, N_sets, ens_name, i)

        T, RF, C, a_C, a_m, Emm, TE_whole, TE, tip_prob_total = run_ssp_stab_an(ssp, i, use_TE_model, thermal_set,
                                                                                gas_set,
                                                                                extforc_sfs,
    ...
    quant_T = pd.concat(quant_T, axis=1, keys=ens)
    quant_RF = pd.concat(quant_RF, axis=1, keys=ens)
    quant_C = pd.concat(quant_C, axis=1, keys=ens)
    quant_alpha_C = pd.concat(quant_alpha_C, axis=1, keys=ens)
    quant_alpha_meth = pd.concat(quant_alpha_meth, axis=1, keys=ens)
    Emm_ssp = pd.concat(Emm_ssp, axis=1, keys=ens)
    quant_TE = pd.concat(quant_TE, axis=1, keys=ens)
    total_TE = pd.concat(total_TE, axis=1, keys=ens)
...
def main():
    ...
    # Run fair ensemble
    run_fair_stab_an(choose_ssps, elements, N_sets, N_ens, ens_name, use_TE_model)
